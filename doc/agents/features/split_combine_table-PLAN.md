...
## Acceptance Criteria
- [x] 部分拆台：可选择部分菜品移至目标桌台，源/目标数量正确更新。
- [x] 整单拆台：源桌台清空，目标桌台完整接收；无孤儿记录。
- [x] 部分并台：可选择目标桌台菜品并入当前桌台，数量正确。
- [x] 整单并台：目标桌台清空，当前桌台累加；数据一致。
- [x] UI 保持现状（无布局/视觉变化），新增逻辑通过子组件封装，`components/pos-interface.tsx` 体量可控。
- [x] 异常防护：桌台不存在/无菜品/重复操作时给予可理解提示；关键操作有确认或禁用态。
- [x] 可观测性：关键流程添加日志/错误提示；若有 API，返回状态明确。
