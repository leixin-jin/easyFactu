...
## Acceptance Criteria
- [x] 右上角新增“增加菜品/+”与“删除菜品/-”按钮，布局与尺寸保持不变。
- [x] 新增菜品弹窗校验必填项（name/category/price），提交成功写入数据库并更新列表；错误提示清晰。
- [x] 删除菜品弹窗可选择现有菜品，成功将该菜品标记为不可用（`available=false`）；被引用或无效 ID 时给出错误提示并不崩溃（提示风格对齐桌台删除接口）。
- [x] API 使用 Drizzle 访问 `menuItems`，`GET` 仅返回 `available = true`，返回 JSON，数值字段为 number，含合理的状态码（400/409/500）。
- [x] 文档引用项目 guides，交互与 UI 风格与现有页面一致，无页面尺寸或主要布局变更。
...
### Task 1 · 字段与交互稿
- `menu_items` 字段与校验：`name`、`category`、`price` 必填（`price` 为正数且最多两位小数）；可选字段包括 `nameEn`、`cost`（≥0、两位小数）、`description`、`image`、`available`（默认 `true`）、`popular`（默认 `false`）、`spicy`（0–5）、`allergens`（`text[]`）。
- 新增弹窗：沿用现有宽度，包含上述字段与开关，错误信息贴近输入框显示，并在提交态禁用按钮 + 显示 `Loader2`。
- 删除弹窗：通过下拉列出当前 `available=true` 的菜品（显示“分类 + 名称”），无可删除项时禁用/提示，提交前校验是否选中。
- 交互：右上角新增“+ 增加菜品”“- 删除菜品”按钮，与既有按钮尺寸一致；所有对话框关闭时重置表单，成功后触发 `useMenuData.refresh()` 重拉菜单。

### Task 2 · `POST /api/menu-items`
- 使用 Zod 构建校验模板（字符串 trim、金额字段校验两位小数、`spicy` 0–5、`allergens` 为去空的字符串数组）。
- `GET` 端新增 `toMenuItemResponse` 映射，统一将 numeric 字段转为 `number`，只返回 `available=true` 的记录，并在响应中附带分类计数。
- `POST` 端复用 `lib/db.ts` + Drizzle，一旦碰到同分类同名的在售菜品返回 409（`MENU_ITEM_EXISTS`），其余校验错误 400，落库前使用 `toMoneyString` 落金额字段。

### Task 3 · `DELETE /api/menu-items/[id]`
- 新建路由并使用 Zod 校验 UUID。若菜单项不存在或已软删除返回 404。
- 删除前查询 `order_items` 是否存在引用，存在则返回 409（`MENU_ITEM_IN_USE`），否则执行软删除（`available=false`、更新 `updated_at`），并返回格式化后的菜单项 JSON。

### Task 4 · 前端 Dialog 接入
- `components/menu-management.tsx` 引入 `useMenuData.refresh()`，新增顶部双按钮、加载 skeleton、空态信息与 toast 提示。
- 新增弹窗采用 shadcn 组件（Input/Textarea/Switch/Select）完成字段输入，提供校验错误文案，提交时禁用按钮并在成功后关闭弹窗 + 刷新。
- 删除弹窗支持下拉选择菜品；请求失败时展示服务器返回或兜底文案；成功后 toast 提示并更新列表。

### Task 5 · 验收要点
- 运行 `pnpm dev` 前需在 `.env.local` 中配置 `DATABASE_URL`、`NEXT_PUBLIC_SUPABASE_URL`、`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`。
- QA 验证重点：列表总数与分类计数实时更新；新增/删除成功后 UI 自动刷新；API 确保 400/409/500 错误各自带原因；`DELETE` 对被订单引用的菜品返回 409。
- （未在本地运行 UI，由于指令环境限制，提交前已检查 TypeScript/ESLint 无报错，推荐在本地运行 `pnpm lint && pnpm dev` 做最终验证。）
